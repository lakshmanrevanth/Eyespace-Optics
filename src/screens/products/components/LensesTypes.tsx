...
const IconComponent: React.FC<{ type: LensType["icon"] }> = ({ type }) => {
  return (
    <div className="w-16 h-16 mx-auto mb-4">
      {type === "single" && (
        <svg viewBox="0 0 24 24" className="w-full h-full text-[#7A6F5C]">
          <rect
            x="4"
            y="4"
            width="16"
...
const Card: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return (
    <div className="p-6 bg-white border border-[#7A6F5C]/30 rounded-lg shadow-sm hover:shadow-md transition-shadow">
      {children}
    </div>
  );
};
  ...
  return (
    <div className="w-11/12 max-w-7xl mx-auto py-8 space-y-10">
      ...
      <div className="text-center space-y-3">
        <h1 className="text-3xl md:text-4xl font-bold text-[#7A6F5C]">
          SPECTACLE AND PRESCRIPTION SUNGLASS LENSES
        </h1>
        <h2 className="text-lg md:text-xl font-medium text-[#7A6F5C]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {lensTypes.map((lens, index) => (
          <Card key={index}>
            ...
            <h3 className="text-lg text-center font-semibold text-[#7A6F5C] mb-2">
              {lens.title}
            </h3>
            <p className="text-sm text-[#7A6F5C]/90 leading-relaxed">
              {lens.description}
            </p>
          </Card>
        ))}
