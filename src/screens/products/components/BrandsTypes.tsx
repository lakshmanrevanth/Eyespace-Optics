      ...
      <table className="w-full border-collapse text-center bg-white rounded-lg overflow-hidden">
        ...
        <tbody className="text-[#6B4F4F]">
          {[
            ...
            <tr
              key={brand.toString()}
              className={`border-b border-[#EDEDED] ${String(index % 2 === 0 ? "bg-[#F9F9F9]" : "bg-white")}`}
            >
              <td className="px-6 py-4 border-r border-[#D4C7C7]">{brand}</td>
              <td className="px-6 py-4 border-r border-[#D4C7C7]">
                {singleVision ? (
                  <Check className="mx-auto text-[#6B4F4F]" />
                ) : (
                  "✘"
                )}
              </td>
              ...
              <td className="px-6 py-4">
                {bifocal ? <Check className="mx-auto text-[#6B4F4F]" /> : "✘"}
              </td>
            </tr>
          ))}
        </tbody>
